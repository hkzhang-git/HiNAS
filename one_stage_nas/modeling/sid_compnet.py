...
def get_genotype_from_adl(cfg):
    ...
    adl_cfg.merge_from_list(['MODEL.META_ARCHITECTURE', 'Sid_supernet',
                             'MODEL.FILTER_MULTIPLIER', 8,
                             'MODEL.AFFINE', True,
                             'SEARCH.SEARCH_ON', True])

    model = Sid_supernet(adl_cfg)
    # load weights
...
class DeepLabScaler_Width(nn.Module):
    ...
    """
    def __init__(self, scale, inp, C, activate_f='ReLU'):
...
class Sid_compnet(nn.Module):
    def __init__(self, cfg):
        super(Sid_compnet, self).__init__()
        ...
        for layer, (geno, h) in enumerate(zip(geno_cell, geno_path), 1):
            ...
            if layer == 1:
                scaler0 = DeepLabScaler(h_0 - h, inp0,
                                        stride * self.f, activate_f=self.activate_f)
    ...
    def forward(self, images, targets=None, drop_prob=-1):
        ...
        if self.use_res and self.res == 'add':
            pred = input_rgb - pred
        elif self.use_res and self.res == 'mul':
            pred = input_rgb * pred
