...
class Router_Width(nn.Module):
    ...
    def __init__(self, ind, inp, C, num_strides=4, affine=True, ws=[1,1,1]):
        """
        Arguments:
        ...
        if ind > 0:
            # downsample
            ind_sf = self.ws[ind]/self.ws[ind-1]
            self.postprocess0 = conv1x1_bn(inp, int(C // ind_sf), 1, affine=affine, activate_f=None)

        self.postprocess1 = conv1x1_bn(inp, C, 1, affine=affine, activate_f=None)
        if ind < num_strides - 1:
            # upsample
            ind_sf = self.ws[ind+1] / self.ws[ind]
            self.postprocess2 = conv1x1_bn(inp, int(C * ind_sf), 1, affine=affine, activate_f=None)
...
class Sr_supernet(nn.Module):

    def __init__(self, cfg):
        super(Sr_supernet, self).__init__()
        ...
        for i in range(1, self.num_strides):
            self.upsamplers.append(conv1x1_bn(int(self.f * self.ws_factors[i - 1]),
                                              int(self.f * self.ws_factors[i]),
        ...
        for l in range(1, self.num_layers + 1):
            for h in range(min(self.num_strides, l + 1)):
                stride = self.ws_factors[h]
                C = int(self.f * stride)

                if h < l:
                    self.routers.append(Router(h, C * self.num_blocks,
                                               C, num_strides=self.num_strides, affine=affine, ws=self.ws_factors))
    ...
    def forward(self, images, targets=None):
        ...
        if self.use_res:
            # pred = F.interpolate(images, scale_factor=self.s_factor, mode='bicubic') + pred
            pred = F.interpolate(images, scale_factor=self.s_factor, mode='bilinear') + pred
