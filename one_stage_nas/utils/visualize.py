...
def model_visualize(model, save_dir, tie=True, return_cell=False):
    geno_cell, geno_path = copy.deepcopy(model).module.cpu().genotype()
    visualize(geno_cell, geno_path, save_dir, tie=tie)

    if return_cell:
        return geno_cell

def visualize(geno_cell, geno_path, save_dir, tie=True):
    g = Digraph(
        format='png',
    ...
    # path
    arch = []
    for layer_num, width in enumerate(geno_path):
        cell = 'cell:{} w:{}'.format(layer_num, math.pow(2, width))
        arch.append(cell)

    g.node(name=' |-->| '.join(arch), fillcolor='lightyellow')
